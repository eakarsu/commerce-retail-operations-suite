...
const adminCookie = await login('admin@commerce-retail.local', 'admin123');
const managerCookie = await login('manager@commerce-retail.local', 'manager123');
const analystCookie = await login('analyst@commerce-retail.local', 'analyst123');

await expectJson('/api/dashboard', adminCookie);
await expectJson('/api/entities/orders', analystCookie);
await expectJson('/api/documents', analystCookie);
await expectStatus('/api/documents/upload', analystCookie, 405);

const records = await expectJson('/api/entities/orders', managerCookie);
const rowId = records.rows[0].id;

const approveResponse = await fetch(`${baseUrl}/api/entities/orders/approve`, {
  method: 'POST',
  headers: {
...
const forbiddenApprove = await fetch(`${baseUrl}/api/entities/orders/approve`, {
  method: 'POST',
  headers: {
